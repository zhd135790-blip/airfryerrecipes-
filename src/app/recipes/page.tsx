        ...
        <div className="container relative z-10 py-20">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl md:text-2xl text-white/95 max-w-3xl mx-auto mb-8 drop-shadow">
              {t('recipes.exploreCollection').replace('{count}', recipes.length.toString())}
            </p>
            ...
            <div className="flex flex-wrap justify-center gap-6 text-white">
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-3 shadow-lg">
                ...
                <div className="text-sm opacity-90">{t('recipes.recipes')}</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-3 shadow-lg">
                <div className="text-3xl font-bold">10+</div>
                <div className="text-sm opacity-90">{t('filter.category')}</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-3 shadow-lg">
                <div className="text-3xl font-bold">100%</div>
                <div className="text-sm opacity-90">{t('recipes.tested')}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="py-8 bg-gray-50">
        <div className="container">
          <div className="flex flex-wrap gap-3 justify-center">
            {(language === 'zh' 
              ? ['全部', '早餐', '肉类和禽类', '海鲜', '蔬菜', '甜点', '素食', '快手菜']
              : ['All', 'Breakfast', 'Meat & Poultry', 'Seafood', 'Vegetables', 'Desserts', 'Vegan', 'Quick Meals']
            ).map((category) => (
              <button
                key={category}
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center p-8 md:p-12">
              <div className="text-white">
                <div className="inline-block bg-yellow-400 text-gray-900 text-xs font-bold px-3 py-1 rounded-full mb-4">
                  ⭐ {t('recipes.featuredCollection')}
                </div>
                <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">
                  {t('recipes.masterAirFryer')}
                </h2>
                <p className="text-lg text-blue-100 mb-6">
                  {t('recipes.masterDesc')}
                </p>
                <div className="flex gap-4">
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                    <div className="text-2xl font-bold">95%</div>
                    <div className="text-xs opacity-90">{t('recipes.successRate')}</div>
                  </div>
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                    <div className="text-2xl font-bold">20min</div>
                    <div className="text-xs opacity-90">{t('recipes.avgTime')}</div>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-16 bg-gradient-to-r from-green-500 to-teal-600 rounded-3xl p-8 md:p-12 text-center shadow-2xl">
            <div className="max-w-3xl mx-auto">
              ...
              <p className="text-xl text-green-50 mb-8">
                {t('recipes.cantFindDesc')}
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <input
                  type="email"
                  placeholder={t('recipes.enterEmail')}
